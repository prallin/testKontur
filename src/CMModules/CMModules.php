...
/**
 * A model for managing Kontur modules.
 *
 * @package konturCore
 */
class CMModules extends CObject {
     ...
     * Properties
     */
    private $KonturCoreModules = array('CKontur', 'CDatabase', 'CRequest', 'CViewContainer', 'CSession', 'CObject');
    private $KonturCMFModules = array('CForm', 'CCPage', 'CCBlog', 'CMUser', 'CCUser', 'CMContent', 'CCContent', 'CFormUserLogin', 'CFormUserProfile', 'CFormUserCreate', 'CFormContent', 'CHTMLPurifier');
    ...
    public function AvailableControllers() {
        ...
        krsort($controllers, SORT_LOCALE_STRING);
        return $controllers;
    }
     ...
     * @return boolean false if $src can not be opened.
     */
    public function ReadAndAnalyse() {
        $src = KONTUR_INSTALL_PATH . '/src';
        if (!$dir = dir($src))
    ...
    public function GetDetailsOfModule($module) {
        ...
        if (class_exists($module)) {
            ...
            $details['hasSQL'] = $rc->implementsInterface('CHasSQL');
            $details['isManageable'] = $rc->implementsInterface('IModule');
            $details['isKonturCore'] = in_array($rc->name, $this->KonturCoreModules);
            $details['isKonturCMF'] = in_array($rc->name, $this->KonturCMFModules);

...
class CKontur implements ISingleton {
    ...
    public function ThemeEngineRender() {
        ...
        //get the paths and settings for the theme, look in the site folder first,
        $themePath = KONTUR_INSTALL_PATH . '/' . $this->config['theme']['path'];
        ...
        if (isset($this->config['theme']['parent'])) {
            $parentPath = KONTUR_INSTALL_PATH . '/' . $this->config['theme']['parent'];
            $parentUrl = $this->request->base_url . $this->config['theme']['parent'];
        }
        // Add stylesheet name to the $kontur->data array
        $this->data['stylesheet'] = $this->config['theme']['stylesheet'];

        // Make the theme urls available as part of $kontur
        ...
        $kontur = &$this;
        // First the default Kontur themes/functions.php
    ...
    public function CreateUrl($urlOrController = null, $method = null, $arguments = null) {
      ...
      /**
   * Draw HTML for a menu defined in $kontur->config['menus'].
   *
